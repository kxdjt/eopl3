...
(define value-of/k
  (lambda (exp senv cont)
    (debug-trace "value-of/k" "exp: cont:~s\n" (print-cont-size cont))
    (let* ((store (cdr senv))
           (env (car senv))
...
(define apply-cont
  (lambda (cont aw)
    (let ((next-cont (cdr cont))
          (args (cdar cont))
          (deal-fun (caar cont)))
      (apply deal-fun aw next-cont args))))
;; continuations --  representing as lists ((fun,args),cont)
(define end-cont
  (lambda ()
    (cons
     (cons
          ...
          (if (null? exps)
              ...
              (value-of/k (car exps)
                          ...
                          (let-cont (cdr vars) (cdr exps) body cont ori-env
                                    (car new-senv))))))
      (list vars exps body ori-env env))
     cont)))
(define cond-cont
  ...
  (lambda (cont)
    (let ((next-cont (cdr cont))
      ...
      (list args
            (if (null? next-cont)
                "end-cont"
                (print-cont next-cont))))))
(define print-cont-size
  (lambda (cont)
    (let ((next-cont (cdr cont)))
      (if (null? next-cont)
          1
          (+ 1 (print-cont-size next-cont))))))
